...
# current column numbers for orgcomp (3), manure (4), cover_crop (5), crop_rotation (6), apm(7), production diversity (8) direct_planting(9)
...
ae_scale_chem <- df_aei[,c(4:9,16)]
alpha(ae_scale_chem) #o.49

# new scae that looks pretty good. ## used. 
...
#Creating scores
ae_score <- ((df_aei$apm +df_aei$crop_rotation + df_aei$direct_planting + df_aei$manure + df_aei$cover_crop + df_aei$production_diversity)/6) # testing one score set 

str(agscores)

# bind the scores onto the original dataframe but store as a new dataframe
df_aei_score <- cbind(df_aei, ae_score) 
df_aei_score
write.csv(df_aei_score, file = "df_aei_score.csv")
```
...
lm_sep1 <- lm(ae_score ~ low_income + credit_access + extension+ farmers_association + only_family_labour + off_farm_income, df_aei_score)
summary(lm_sep1)
```
...
pl_cred <- ggplot(df_aei_score, aes(x=agscores, y = credit_access)) +
  geom_point() +
  geom_smooth(method=lm)

pl_assoc <- ggplot(df_aei_score, aes(x=agscores, y = farmers_association)) +
  geom_point() +
  geom_smooth(method=lm)

pl_extension<- ggplot(df_aei_score, aes(x=agscores, y = extension)) +
  geom_point() +
  geom_smooth(method=lm)

pl_low_income <- ggplot(df_aei_score, aes(x=agscores, y = low_income)) +
  geom_point() +
  geom_smooth(method=lm)

pl_offfarm <- ggplot(df_aei_score, aes(x=agscores, y = off_farm_income)) +
  geom_point() +
  geom_smooth(method=lm)

pl_only_family_labour <- ggplot(df_aei_score, aes(x=agscores, y = only_family_labour)) +
  geom_point() +
  geom_smooth(method=lm)
...
stres_a <- ggplot(df_aei_score, aes(farmers_association, st_res)) +
  geom_point() +
  stat_smooth()

stres_c <- ggplot(df_aei_score, aes(credit_access, st_res)) +
  geom_point() +
  stat_smooth()

stres_i <- ggplot(df_aei_score, aes(low_income, st_res)) +
  geom_point() +
  stat_smooth()

stres_t <- ggplot(df_aei_score, aes(extension, st_res)) +
  geom_point() +
  stat_smooth()

stres_fl <- ggplot(df_aei_score, aes(only_family_labour, st_res)) +
  geom_point() +
  stat_smooth()

stres_of <- ggplot(df_aei_score, aes(off_farm_income, st_res)) +
  geom_point() +
  stat_smooth()
...
df_aei_score[c(10,29,58,105,273), 1:2]
...
df_diag_sep <- cbind(df_aei_score, cd, sres) # add cooks D and student residuals to the dataframe
...
```{r}
prob_cases <- df_aei_score[c(10, 21,29,58,77,145,246,258,273), 1:2]
...
lm_sep_rr <- lm(ae_score ~ low_income + credit_access + extension + farmers_association + only_family_labour + off_farm_income, rerun_df_aei_scores)
summary(lm_sep_rr)
```
...
```{r}
#COVER CROP 
lm_cover_crop_rr <- lm(cover_crop ~ low_income + credit_access + extension+ farmers_association + only_family_labour + off_farm_income, rerun_df_aei_scores)
summary(lm_cover_crop_rr)

#CROP ROTATION 
lm_crop_rotation_rr <- lm(crop_rotation ~ low_income + credit_access + extension+ farmers_association + only_family_labour + off_farm_income, rerun_df_aei_scores)
summary(lm_crop_rotation_rr)

#MANURE
lm_manure_rr <- lm(manure ~ low_income + credit_access + extension+ farmers_association + only_family_labour + off_farm_income, rerun_df_aei_scores)
summary(lm_manure_rr)

#APM
lm_apm_rr <- lm(apm ~ low_income + credit_access + extension+ farmers_association + only_family_labour + off_farm_income, rerun_df_aei_scores)
summary(lm_apm_rr)

#production_diversity
lm_production_diversity_rr <- lm(production_diversity ~ low_income + credit_access + extension + farmers_association + only_family_labour + off_farm_income, rerun_df_aei_scores)

#direct_planting
lm_direct_planting_rr <- lm(direct_planting ~ low_income + credit_access + extension+ farmers_association + only_family_labour + off_farm_income, rerun_df_aei_scores)

#no-chem
lm_no_agrichemicals_rr <- lm(no_agrichemicals ~ low_income + credit_access + extension+ farmers_association + only_family_labour + off_farm_income, rerun_df_aei_scores)
summary(lm_no_agrichemicals)
...
#Aggregated only 
stargazer(lm_sep1, type = "text", title="Aggregated Practices Model", digits=1,dep.var.labels=c("Aggregated Score"), covariate.labels = c("Low income", "Credit access", "Extension",  "Farmers' association", "Only family labour", "Off farm income"),  out="Aggregate_reg_table_05.11.16.txt")

#Disaggregated only 
stargazer(lm_cover_crop, lm_crop_rotation, lm_direct_planting, lm_production_diversity, lm_apm, lm_no_agrichemicals, type = "text", title="Disaggregated Practices Model", digits=1, dep.var.labels=c("Cover crop", "Crop rotation", "Direct planting", "Production diversity", "APM", "No agrichemicals"), covariate.labels = c("Low income", "Credit access", "Extension",  "Farmers' association", "Only family labour", "Off farm income"), out ="Disaggregate_reg_table_05.11.16.txt")

# NOTE: STEPWISE IS IN OTHER FILE 

# Descriptive statistics 
df_aei_score_dep <- df_aei_score[,c(5:9,16:17)]
stargazer(df_aei_score_dep, type = "text", column.labels =c("Cover crop", "Crop rotation", "Direct planting", "Production diversity", "APM", "No agrichemicals", "Aggregated score"), out ="Descriptive_dep_05.10.16.txt")

df_aei_score_indep <- df_aei_score[,c(10:15)]
stargazer(df_aei_score_indep, type = "text", out ="Descriptive_indep_05.10.16.txt")


####OLD 

# stargazer(lm_cover_crop, lm_crop_rotation, lm_direct_planting, lm_production_diversity, lm_apm, lm_no_agrichemicals, type = "text", title="Linear Model - FULL ", digits=1, out="Regression_Tables_Individ_Indep_04.26.16.txt")

# # Table with all the individual and the aggregate score 
# stargazer(lm_cover_crop, lm_crop_rotation, lm_direct_planting, lm_production_diversity, lm_apm, lm_no_agrichemicals, lm_sep1, type = "text", title="Disaggregated & Aggregated Models ", digits=1, dep.var.labels=c("Cover crop", "Crop rotation", "Direct planting", "Production diversity", "APM", "No agrichemicals", "Aggregated score"), covariate.labels = c("Low income", "Credit access", "Extension",  "Farmers' association", "Only family labour", "Off farm income"), out ="Regression_Tables_DandA_05.10.16.txt")

##ReRUN tests : Note that there are no substantial differences so it doesn't matter. 
# stargazer(lm_sep_rr, lm_sep1, type = "text", title="Linear Model - FULL ", digits=1, out="Rerun_vs_Original_04.26.16.txt")
# 
# stargazer(lm_cover_crop_rr, lm_crop_rotation_rr, lm_direct_planting_rr, lm_production_diversity_rr, lm_apm_rr, lm_no_agrichemicals_rr, type = "text", title="Linear Model - FULL ", digits=1, out="Rerun_vs_Original_Individ_Indep_04.26.16.txt")
```


```{r}
cor(df_aei$direct_planting, df_aei$no_agrichemicals)
