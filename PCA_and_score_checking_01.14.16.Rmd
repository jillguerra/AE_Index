...
df_aei <- df_aei %>% 
  na.omit() # take out observations that have an NA in any variable 
...
# log transform 
log_ag_var <- log(df_aei[,3:9]) # columns with the AE components going to be used in the index 
...
```{r}
#Create the index
agscores <- ((df_aei$apm +df_aei$croprot + df_aei$straw)/3) # testing one score set 

df_aei_scores <- cbind(df_aei, agscores) # bind the scores onto the original dataframe 
df_aei_scores

lm_PC1_test <-lm(agscores ~ lowincome + credit + associated + regtech + offfarminc + diverse + famlab, df_aei_scores) 
